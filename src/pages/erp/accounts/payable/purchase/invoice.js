...
import { payablePurchase } from "../../../../../validation/schema";
...
const Index = () => {
  ...
  const fetchPurchase = useCallback(async () => {
    try {
      const result = await handleFetch("GET", `/purchase/${purchaseId}`);
      if (result.purchase) {
  ...
  const initialValues = {
    ...
    purchase: paymentDetail?.purchase || purchaseId,
    tax: paymentDetail?.tax || 0,
    discount: paymentDetail?.discount || 0,
    paymentDate: paymentDetail?.paymentDate
      ? new Date(paymentDetail?.paymentDate).toISOString().split("T")[0]
      : new Date().toISOString().split("T")[0],
  };

  const formik = useFormik({
    initialValues,
    validationSchema: payablePurchase,
  ...
  const isDisable = paymentDetail?.status === "Complete" ? true : false;
        ...
        <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row gap-2">
          <div className="w-full sm:w-1/2">
            ...
            <input
              ...
              name="supplier"
              type="text"
              value={`${data?.supplierId?.name} (${data?.supplierId?.code})`}
              readOnly
          ...
          <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row gap-2">
            ...
            <div className="w-full sm:w-1/2">
              ...
              <input
                ...
                disabled={isDisable}
              />
              <FormikError formik={formik} fieldName={"paymentDate"} />
            </div>
          </div>
