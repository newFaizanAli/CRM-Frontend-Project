...
const Index = ({ handleFetch, ModalWrapper, FilterBox }) => {
  ...
  const fetchData = async () => {
    const resp = await handleFetch(
      "GET",
      `/erp/buying/dashboard/purchase/${filterValue?.supplier}/${filterValue?.startDate}/${filterValue?.endDate}`
    );
      ...
      <div className="w-full p-4 rounded-2xl shadow-sm border border-gray-300">
        ...
        <ResponsiveContainer width="100%" height={200}>
          {chartData.length > 0 ? (
            ...
            <div className="text-center text-gray-400">No data available</div> // Fallback if no data
          )}
        </ResponsiveContainer>
      </div>
    </>
