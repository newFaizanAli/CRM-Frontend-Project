...
const Index = ({ handleFetch, suppliers, ModalWrapper, FilterBox }) => {
  ...
  const fetchData = async () => {
    const resp = await handleFetch(
      "GET",
      `/erp/buying/dashboard/purchase/amount/${filterValue?.supplier}/${filterValue?.startDate}/${filterValue?.endDate}/${filterValue?.purStatus}`
    );
        ...
        <div>
          {chartData.length > 0 ? (
            <div className="relative">
              ...
              {/* Show message when there are no payments */}
